...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='thunder',
        choices=['0.5x', '1.0x', '1.5x', '2.0x', 'thunder'],
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=use_gpu
    )
    ...
    optimizer = torch.optim.SGD(
        get_parameters(model), lr=args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay
    )
    ...
    while all_iters < args.total_iters:
        ...
        validate(model, device, args, all_iters=all_iters)
    all_iters = train(
        model, device, args, val_interval=int(1280000 / args.batch_size), bn_process=True, all_iters=all_iters
    )
    validate(model, device, args, all_iters=all_iters)
    save_checkpoint(
        {
...
def train(model, device, args, *, val_interval, bn_process=False, all_iters=None):
    ...
    for iters in range(1, val_interval + 1):
        ...
        if all_iters % args.display_interval == 0:
            printInfo = (
                'TRAIN Iter {}: lr = {:.6f},\tloss = {:.6f},\t'.format(
                    all_iters, scheduler.get_last_lr()[0], loss.item()
                )
...
def validate(model, device, args, *, all_iters=None):
    ...
    logInfo = (
        'TEST Iter {}: loss = {:.6f},\t'.format(all_iters, objs.avg)
